...
## 任务详情
- [ ] 为所有接收用户输入的 IPC handler 定义 zod schema 并 `parse`/`safeParse`:
...
- [ ] 用一个 `withIpcHandler(name, schema, fn)` 包装器统一处理:校验 → 执行 → try-catch。
...
## 验收 / 测试标准
- [ ] 单测:对每个 schema 喂合法/非法样本,校验非法被拒(缺字段、类型错误、超长)。
- [ ] 单测:包装器在 fn 抛错时返回通用错误且不含敏感字段;在校验失败时不调用 fn。
- [ ] 构造非法 IPC 调用(如 `add-password` 传缺字段对象),应被拒绝且不写库。
- [ ] `tsc`/`eslint` 通过;现有正常流程(增删改查、导入)回归无变化。
