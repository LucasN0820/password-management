...
export const PasswordContext = createContext<UseBoundStore<StoreApi<PasswordState>>
>(undefined!)
  ...
  const store = create<PasswordState>((set, get) => ({
    ...
    addPassword: async (data) => {
      ...
      try {
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
      `, data.title, data.username, data.password, data.url, data.notes, data.category, data.favorite, data.icon ?? '')
    ...
    updatePassword: async (id, data) => {
      ...
      try {
        ...
        WHERE id = ?
      `, data.title, data.username, data.password, data.url, data.notes, data.category, data.favorite, data.icon ?? '', id)
    ...
    findPassword: async (id) => {
      ...
      try {
        const result = await db.getFirstAsync<Password>(`SELECT * FROM passwords WHERE id = ?`, id)
