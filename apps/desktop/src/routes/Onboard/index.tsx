import { Bot, CheckCircle2, FileText, Loader2, Save, Sparkles, Trash2, Upload, XCircle } from 'lucide-react'
  ...
  const handleSave = async () => {
    try {
      ...
      toast({
        title: 'Save failed',
        description:
          saveError instanceof Error ? saveError.message : 'Unable to save imported credentials.',
        variant: 'destructive',
      })
    }
  }
    ...
    <div className="h-full overflow-y-auto bg-[linear-gradient(180deg,#fff_0%,#faf8f3_100%)]">
      <div className="mx-auto flex max-w-6xl flex-col gap-6 px-10 py-10">
        <section className="rounded-[28px] border border-border bg-white/90 p-8 shadow-sm">
          <div className="flex items-start justify-between gap-8">
            <div className="max-w-2xl">
              <div className="mb-4 inline-flex items-center gap-2 rounded-full bg-[#EEF8F3] px-3 py-1 text-sm font-medium text-[var(--accent-green)]">
                <Bot className="h-4 w-4" />
                AI Onboard MVP
              </div>
              <h1 className="font-heading text-[44px] leading-tight text-foreground">
                Import passwords from mixed files and review them before saving.
              </h1>
              <p className="mt-3 max-w-xl text-base text-muted-foreground">
                Upload password exports, notes, scans, or screenshots. The desktop app parses
                them in the background, extracts likely credentials, and lets you fix anything
                before it reaches the database.
              </p>
            </div>
            <div className="hidden min-w-[260px] rounded-3xl border border-border bg-[#F7F6F3] p-5 lg:block">
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            ...
            <Button onClick={handleRunImport} disabled={!files.length || stage === 'processing'}>
              {stage === 'processing' ? (
                <>
          ...
          {error ? (
            <div className="mt-4 rounded-2xl border border-[color:rgba(235,87,87,0.18)] bg-[rgba(235,87,87,0.08)] px-4 py-3 text-sm text-[var(--accent-red)]">
              {error}
            </div>
          ) : null}
        </section>
        ...
        <section className="grid gap-6 xl:grid-cols-[320px_minmax(0,1fr)]">
          <Card className="gap-0 overflow-hidden border-border/80 bg-white/95">
            ...
            <CardContent className="space-y-3 pt-6">
              {files.length ? (
                files.map(file => {
                  const result = fileResults.find(item => item.fileName === file.name)
                  return (
                    <div
                      key={file.path}
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        ...
                        {result?.status === 'processed' ? (
                          ...
                          <XCircle className="h-4 w-4 shrink-0 text-[var(--accent-red)]" />
                        ) : (
            ...
            <CardHeader className="border-b border-border/70">
              <div className="flex items-center justify-between gap-4">
                <div>
                  ...
                  <CardDescription>
                    Edit any field, uncheck entries you do not want, then save the rest.
                  </CardDescription>
                </div>
                <div className="rounded-full bg-[#F7F6F3] px-3 py-1 text-sm text-muted-foreground">
            ...
            <CardContent className="space-y-4 pt-6">
              {stage === 'processing' ? (
                <div className="flex min-h-[360px] flex-col items-center justify-center gap-4 text-center">
                  <div className="flex h-16 w-16 items-center justify-center rounded-full bg-[#F7F6F3]">
                ...
                <>
                  {warnings.length ? (
                    <div className="rounded-2xl border border-[color:rgba(245,197,66,0.3)] bg-[#FFF8E1] px-4 py-3 text-sm text-[#8A6D1D]">
                      {warnings.join(' · ')}
                    </div>
                  ) : null}
                  ...
                  <div className="space-y-4">
                    {candidates.map(candidate => (
                      <div
                        key={candidate.id}
                        className="rounded-[24px] border border-border bg-[#FCFBF8] p-5"
                      >
                        ...
                        <div className="grid gap-4 md:grid-cols-2">
                          <div className="space-y-2">
                            <Label htmlFor={`${candidate.id}-title`}>Title</Label>
                            <Input
                          ...
                          <div className="space-y-2">
                            <Label htmlFor={`${candidate.id}-url`}>Website</Label>
                            <Input
                              id={`${candidate.id}-url`}
                              onChange={event =>
                          ...
                          <div className="space-y-2">
                            <Label htmlFor={`${candidate.id}-username`}>Username / Email</Label>
                            <Input
                          ...
                          <div className="space-y-2">
                            <Label htmlFor={`${candidate.id}-password`}>Password</Label>
                            <Input
                          ...
                          <div className="space-y-2 md:col-span-2">
                            <Label htmlFor={`${candidate.id}-notes`}>Notes</Label>
                            <textarea
                              className="min-h-[88px] w-full rounded-xl border border-input bg-background px-3 py-2 text-sm shadow-xs outline-none transition focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50"
                        ...
                        <div className="mt-4 rounded-2xl bg-white px-4 py-3 text-sm text-muted-foreground">
                          ...
                          <div>{candidate.sourceExcerpt || `Detected from ${candidate.sourceFile}`}</div>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <div className="flex min-h-[360px] flex-col items-center justify-center gap-4 text-center">
                  ...
                  <div>
                    ...
                    <div className="mt-1 text-sm text-muted-foreground">
                      The imported results will appear here for manual confirmation and edits.
                    </div>
                  </div>
                </div>
              )}
