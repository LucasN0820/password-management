...
function scoreChunk(chunk: string) {
  ...
  return (
    keywordMatches + urlMatches * 2 + emailMatches * 2 + separatorMatches
  )
}
...
function detectCsvField(header: string): CredentialField | null {
  ...
  if (
    ['title', 'name', 'service', 'site', 'website name'].includes(normalized)
  ) {
    return 'title'
  }
  if (
...
function buildCsvPrefilledCandidates(
